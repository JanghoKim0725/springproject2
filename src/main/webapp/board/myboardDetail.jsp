...
<html>
 ...
 <script>
 	<%/* 윈도우팝업창: 자바스크립트에서 지원해주는 함수를 이용하여 구현
 		 ...
 		 사용 예) 팝업처럼 사용할 <div></div> 를 보이지 않게 기본 배치한 후 팝업 오픈 버튼을 클릭했을 때 활성화 시킴 */%>
 	...
 	function fn_delete(seqid) {
		...
		// 팝업창 함수 (주소,별칭,옵션)
		window.open(url,"popup1","left="+pw+",top="+ph+",width=500,height=230");
	}
	...
	$(function() {
		...
		$("#btn_before").click(function() {
			if(${dto.bseqid} > 0) location = "/myboardDetail/${dto.bseqid}";
			else alert("첫번째 페이지입니다.");
		});
		
		$("#btn_next"  ).click(function() {
			if(${dto.nseqid} > 0) location = "/myboardDetail/${dto.nseqid}";
			else alert("마지막 페이지입니다.");
		});
		
		$("#btn_list"  ).click(function() {location = "/myboardList";});
		
		$("#btn_modify").click(function() {location = "/myboardModify/${dto.seqid}";});
		
		$("#btn_delete").click(function() {location = "/passWrite2/${dto.seqid}";});
		...
		$("#btn_submit").click(function() {
			...
			let url  = $("#url").val();  // 등록:boardInsert ;; 수정:boardUpdate
			...
			$.ajax({
				...
				success	 : function(data) { 
					...
					else if (data == "2") alert("암호가 일치하지 않습니다.");
					else 				  alert("저장실패");
				},
 ...
 <body>
	<div class="div_title">댓글 게시판 상세화면</div>
	<form id="frm1" >
		<table class="table1">
			<colgroup>
				<col width="20%" />
	...
	<div class="div_comment">
		...
		<form id="frm2">
			...
			<div><textarea id="cmmt" name="cmmt" class="textarea2"></textarea></div>
			<div>		
			...
			<c:forEach var="result" items="${list}">
				<tr>
					...
					<td style="padding:5px;">
						<div id="cmmt${cnt}">${result.CMMT}</div>
						<div style="text-align:right;padding-top:3px;">
							<!-- 황금색별 출력 -->
							<c:forEach var="i" begin="1" end="${result.STAR}">
								<img src="/images/icons/gold_star.png" width="15" height="15">
							</c:forEach>
							<!-- 은색별 출력 -->
							<c:forEach var="i" begin="1" end="${5-result.STAR}">
								<img src="/images/icons/silver_star.png" width="15" height="15">
							</c:forEach>
							${result.WRITER} ${result.RDATE} [
							<a href="#" onClick=
							 "fn_modify('${cnt}','${result.WRITER}','${result.STAR}','${result.SEQID}')">
							 	수정
							</a>/
 ...
 <script>
	$(function() {
		...
		// 황금색별 출력
		let img = "<img src='/images/icons/gold_star.png' width='25' height='25'>";
		// 2.5 -> 2 ,, 3.2 -> 3 
		// 1 ~ 5
		for(var i=1; i<=Math.floor(avg); i++) value += img;
		
		// 절반별 출력
		if(a2 > 0 && len > 1) value += "<img src='/images/icons/half_star.png' width='25' height='25'>";
		...
		// 은색별 출력
		img = "<img src='/images/icons/silver_star.png' width='25' height='25'>";
		for(var i=1; i<=(5-Math.ceil(avg)); i++) value += img;
